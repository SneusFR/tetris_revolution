...
const useThemeStore = create((set, get) => ({
  ...
  fetchThemes: async () => {
    ...
    try {
      ...
      if (response.success) {
        // Les thèmes du serveur ont déjà les couleurs, on les utilise directement
        const themesFromServer = response.data.themes;
        
        // Trouver le thème actuellement sélectionné
        const currentThemeId = response.data.userStats?.currentTheme || 'neon';
        const currentThemeObj = themesFromServer.find(t => t.id === currentThemeId) || 
                               themesFromServer.find(t => t.id === 'neon') ||
                               themesFromServer[0];
